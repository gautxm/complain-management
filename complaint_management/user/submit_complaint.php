    ...
    if (empty($title) || empty($description) || $category_id === 0) {
        ...
        if (!empty($_FILES['attachment']['name'])) {
            ...
            if (!in_array($ext, $allowed)) {
                ...
                // Use absolute path — works on all OS (Windows/Linux)
                ...
                if (move_uploaded_file($_FILES['attachment']['tmp_name'], $uploadPath)) {
                    ...
                    $error = "File upload failed. Make sure the uploads/ folder exists and is writable.";
                }
            }
        }
        ...
        if (empty($error)) {
            ...
            $stmt = $conn->prepare("INSERT INTO complaints
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, 'Pending')");
            $stmt->bind_param("sisisss", $complaint_no, $uid, $title, $category_id, $description, $attachment, $priority);
            if ($stmt->execute()) {
